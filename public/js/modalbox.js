...
Modalbox.Methods = {
	...
	webshopalert: function(message, timeOut, redirectTo){
		var html = '<div class="MB_alert"><p>' + message + '</p></div>';
		Modalbox.show(html, {
			title: 'IU Webshop',
			width: 300,
			onShow: function() {
				setTimeout("Modalbox.hide({ afterHide: function() { setTimeout(\"window.location = '" + redirectTo + "'\", 1000); } })", timeOut*1000);
			}
		});
	},
		...
		if(this.event("beforeLoad") != false) { // If callback passed false, skip loading of the content
			if(typeof this.content == 'string') {
				...
				if(htmlRegExp.test(this.content)) { // Plain HTML given as a parameter
					...
					this._insertContent(this.content, function(){
					...
					new Ajax.Request( this.content, { method: this.options.method.toLowerCase(), parameters: this.options.params, 
						onComplete: function(transport) {
							...
							this._insertContent(transport.responseText, function(){
